...
export default function useEditorDaemon(EditorElementRef, MirrorDocumentRef, FinalizeChanges, Options) {
    // Default options
    const DaemonOptions = Object.assign({ TextNodeCallback: undefined, OnRollback: undefined, ShouldObserve: true, ShouldLog: true, IsEditable: true, ParagraphTags: ParagraphTest, HistoryLength: 10, DebounceSyncDelay: 500 }, Options);
            ...
            if (mutation.type === "characterData" && mutation.oldValue !== null) {
                ...
                if (typeof DaemonOptions.TextNodeCallback === 'function') {
                    ...
                    if (!callbackResult || !callbackResult.length) {
                        ...
                        if (((_a = OldTextNode.textContent) === null || _a === void 0 ? void 0 : _a.trim()) === '' && (mutation.oldValue || TextNodeOriginalValue)) {
    ...
    function GetXPathFromNode(node, bFilterGenerated = true) {
        if (!EditorElementRef.current || !node)
            return '';
        ...
        if (node.className === EditorElementRef.current.className && node.tagName === EditorElementRef.current.tagName) {
            return '//body';
        }
        // text nodes
    ...
    function GetXPathNthChild(node, bFilterGenerated = true) {
        ...
        while (node && node !== document.body) {
            ...
            if (EditorElementRef.current && node.className === EditorElementRef.current.className && node.tagName === EditorElementRef.current.tagName) {
    ...
    function RestoreSelectionStatus(RootElement, SavedState) {
        ...
        try {
            ...
            console.warn("StartingOffset error,", e.message);
            ...
            switch (TokenString) {
                ...
                case 'PrevLine': {
                    while (AnchorNode = Walker.previousNode()) {
                        if (AnchorNode.parentNode !== EditorElementRef.current)
                            continue;
    ...
    return {
        ...
        SetSelectionStatus(status, ShouldOverride = false) {
